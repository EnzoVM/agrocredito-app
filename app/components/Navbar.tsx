...
export default function Navbar () {
  ...
  const sectionSelected = path

  const handlerClick = (event: MouseEvent<HTMLAnchorElement>) => {
    if (sectionSelected === 'campaign') {
    ...
    <nav className="bg-white dark:bg-gray-900 fixed w-full z-20 top-0 left-0 border-b border-gray-200 dark:border-gray-600">
      ...
      <div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-sticky">
        <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
          <li>
            <Link href="/home/campaign" className={isCampaignSelected ? selectedButtonclass : unselectedButtonclass} aria-current="page" onClick={handlerClick}>Campañas</Link>
          </li>
          <li>
            <Link href="/home/farmer"className={isCampaignSelected ? unselectedButtonclass : selectedButtonclass} onClick={handlerClick}>Agricultores</Link>
          </li>
        </ul>
      </div>
      </div>
