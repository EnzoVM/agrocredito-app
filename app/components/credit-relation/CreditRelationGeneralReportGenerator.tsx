...
import CreditRelationGenenalDocument from './CreditRelationGeneralDocument'

export default function CreditRelationGeneralReportGenerator ({ campaignId }: { campaignId: string }) {
  ...
  const getMinutesAndRegisterNewLog = async () => {
    ...
    try {
      const { recordId } = await createLogRecord({
        resource: 'report-credit-relation',
        method: 'GET',
  ...
  return (
    <>
      {
        finishLoadPDFData
          ? <PDFDownloadLink onClick={getMinutesAndRegisterNewLog} document={<CreditRelationGenenalDocument creditRelations={creditRelations} campaignId={campaignId}/>} fileName={`relación_de_créditos_${campaignId}.pdf`} >
              <Button
              ...
              >
                Reporte de relación de créditos
              </Button>
            </PDFDownloadLink>
          : <Button
              disabled 
